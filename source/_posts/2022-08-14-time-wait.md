...
2. client recieve message, in read buffer it sees EOF from sever, so it ACK server immedialtely and change status from
...
4. the client still not able to close connection, because its writing buffer may still has message to send
